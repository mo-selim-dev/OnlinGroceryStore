...
//  OnlinGroceryStore
//
//  Created by Mohamed Selim on 25/02/2025.
//
...
class ForgotPasswordViewModel: ObservableObject
{
    ...
    func serviceCallRequest(){
        ...
        ServiceCall.post(parameter: ["email": txtEmail ], path: Globs.Endpoints.forgotPasswordRequest, isToken: false ) { responseObj in
    ...
    func serviceCallVerify(){
        ...
        ServiceCall.post(parameter: ["email": txtEmail, "reset_code": txtResetCode ], path: Globs.Endpoints.forgotPasswordVerify, isToken: false ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: ResponseKeys.status) as? String ?? "" == "1" {
                    self.resetObj = response.value(forKey: ResponseKeys.payload) as? NSDictionary
    ...
    func serviceCallSetPassword(){
        ...
        ServiceCall.post(parameter: ["user_id": self.resetObj?.value(forKey: "user_id") ?? "", "reset_code":self.resetObj?.value(forKey: "reset_code") ?? "" , "new_password": txtNewPassword], path: Globs.Endpoints.setNewPassword, isToken: false ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: ResponseKeys.status) as? String ?? "" == "1" {
