...
//  OnlinGroceryStore
//
//  Created by Mohamed Selim on 09/02/2025.
//
...
class PaymentViewModel: ObservableObject
{
    ...
    func serviceCallAdd(didDone: ((  )->())? ) {
        ...
        ServiceCall.post(parameter: ["name":  txtName, "card_number": txtCardNumber, "card_month": txtCardMonth, "card_year": txtCardYear  ], path: Globs.Endpoints.addPaymentMethod, isToken: true ) { responseObj in
            if let response = responseObj as? NSDictionary {
                if response.value(forKey: ResponseKeys.status) as? String ?? "" == "1" {
                    self.clearAll()
                    self.serviceCallList()
